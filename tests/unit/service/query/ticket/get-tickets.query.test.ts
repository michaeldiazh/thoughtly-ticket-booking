/**
 * Tests for Get Tickets Query Builder
 */
...
describe('buildAvailableTicketsCountQuery', () => {
  ...
  it('should build COUNT query with no WHERE clause when query has only limit and offset', () => {
    ...
    // Verify SQL structure: should go directly from JOINs to end (no WHERE)
    const sql = result.sql.trim();
    expect(sql).toContain('INNER JOIN venue v ON e.venue_id = v.id');
    expect(sql).not.toContain('WHERE');
    // Verify the SQL ends after the JOINs (no WHERE clause inserted)
    const afterJoinIndex = sql.indexOf('INNER JOIN venue v ON e.venue_id = v.id');
...
describe('buildAvailableTicketsSelectQuery', () => {
  ...
  it('should build SELECT query with no WHERE clause when query has only limit and offset', () => {
    ...
    // Verify SQL structure: should go directly from JOINs to ORDER BY (no WHERE)
    const sql = result.sql.trim();
    expect(sql).toContain('INNER JOIN price_tier pt ON t.tier_code = pt.code');
    expect(sql).not.toContain('WHERE');
    // Verify the SQL flows correctly: JOINs -> ORDER BY (no WHERE in between)
    const joinIndex = sql.indexOf('INNER JOIN price_tier pt ON t.tier_code = pt.code');
    const orderByIndex = sql.indexOf('ORDER BY');
    const betweenJoinAndOrderBy = sql.substring(joinIndex + 'INNER JOIN price_tier pt ON t.tier_code = pt.code'.length, orderByIndex).trim();
